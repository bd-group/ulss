...
public class HttpDataHandler extends AbstractHandler {
        ...
        try {
            ...
            if (action.startsWith("createfile")) {
                synchronized (HttpDataHandler.o) {
                    ...
                    log.info("init lucene file for " + reqID + "<---> " + file_id + " ok,the file location size for it is " + sf.getLocations().size());
                ...
                log.info("create file use " + (System.currentTimeMillis() - bg) + " ms for request id:" + reqID + " <---> " + action);
            } else {
                ...
                synchronized (HttpDataHandler.o) {
                    try { /*不包括这个file_id的话证明对应的create线程已经结束或者还没有开始，那么就要进行创建*/
                        if (!HttpDataHandler.id2Createindex.containsKey(file_id)) {  //与CreateIndex中的HttpDataHandler.key2Createindex.remove(sf.getFid()) 语句同步?
                            ...
                            DataSourceConfig ds = new DataSourceConfig(Indexer.zkUrls, db, tb, stTime);
                            ...
                            CreateIndex c = new CreateIndex(ds, Indexer.ClientAPIMap.get(ds.getDbName()), buf, writer, file_id, HttpDataHandler.o);
                ...
                try {
                    ...
                    GenericData.Array<GenericRecord> tmp = (GenericData.Array<GenericRecord>) docsRecord.get(Indexer.docs_set_name);
                    receiveIntotal.addAndGet(tmp.size());
                    log.info("the receive number total" + reqID + " " + action + " " + receiveIntotal.get() + ",this time receive number " + tmp.size());
                    String schemaName = docsRecord.get(Indexer.docs_schema_name).toString().toLowerCase();
                     ...
                     */
                    CreateIndex createIdx = id2Createindex.get(file_id);
                    if (createIdx == null) {
                        try {
                            hsResonse.setStatus(HttpServletResponse.SC_OK);
                        ...
                        if (id2Createindex.get(file_id).isDiskBad.get()) {
                             ...
                             * fix me
                             */
                            GenericRecord tmpRecord = DataEmiter.getStaticsRecords(schemaName, createIdx.dbName, tmp.size(), "normal", "", "DP_persi_receive");
    ...
    public static HttpDataHandler getDataHandler() {
        ...
        try {
            Protocol protocol = Protocol.parse(Indexer.schemaname2schemaContent.get(Indexer.docs_schema_name));
            HttpDataHandler.docsSchema = protocol.getType(Indexer.docs_schema_name);
