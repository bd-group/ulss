...
public class WriteRawfile implements Runnable {
    ...
    public WriteRawfile(CreateIndex c, String path, String file_location, String devid, String schemaname, long fileid, ArrayBlockingQueue<GenericRecord> buf) {
        this.createIndex = c;
        /*
         */
        this.cachePath = path;
        ...
        this.docsSchema = protocol.getType(Indexer.docs_schema_name);
    ...
    public void run() {
        ...
        String pathInfo = "";
        File f = null;
        List<GenericRecord> innerBuffer = new ArrayList<GenericRecord>(Indexer.writeInnerpoolSize);
        ...
        while (true) {
            if (this.createIndex.isShouldNewRaw.get()) {
                if (fileFullName != null) {
                    WriteRawfile.closeAvroFile(fileFullName);
                }
                isNew = true;
            ...
            this.buffer.drainTo(innerBuffer, Indexer.writeInnerpoolSize);
            ...
            if (this.createIndex.isAllOver.get()) {
                log.info("close the last raw file,and the raw filw writer exit ...");
                closeAvroFile(fileFullName);
                HttpDataHandler.id2WriteRawfile.remove(this.file_id);
                break;
            }
        }
    }

    public static synchronized void writeAvroFile(GenericRecord gr, String fname, boolean isNew) {
        try {
    ...
    public static synchronized void closeAvroFile(String fname) {
        try {
            DatumWriter<GenericRecord> write = new GenericDatumWriter<GenericRecord>(null);
            DataFileWriter<GenericRecord> dataFileWriter = new DataFileWriter<GenericRecord>(write);
