...
public class DataEmiter {
    ...
    public static void initEmiter() {
        DataEmiter.staticsDataemiter = MQProducerPool.getMQProducerPool(Indexer.staticsMq, Indexer.producerPoolSize);
        ...
        for (int i = 0; i < baossPoolSize; i++) {
            ...
            try {
                datumwriterPool.put(new GenericDatumWriter<GenericRecord>(Indexer.statVolumeSchema));
            } catch (Exception ex) {
                log.error(ex, ex);
            }
        }
