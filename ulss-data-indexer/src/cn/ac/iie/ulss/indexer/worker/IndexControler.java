...
public class IndexControler implements Runnable {
    ...
    public void run() {
        ...
        ArrayBlockingQueue<PoolWriter> writerPool = new ArrayBlockingQueue<PoolWriter>(Indexer.luceneWriterMaxParallelNum * tone_up);
        ArrayList<Future> executeResultPool = new ArrayList<Future>(Indexer.luceneWriterMaxParallelNum * tone_up);
        ...
        for (int i = 0; i < Indexer.luceneWriterMaxParallelNum * tone_up; i++) {
        ...
        while (true) {
            ...
            if (!Indexer.isShouldExit.get()) {
                if (bufferEmptyCount >= (1000 / sleepMilis) * Indexer.maxDelaySeconds) { //连续循环检测1200次发现数据都是空的，表明在这段时间内（300秒）都没有数据发送过来，那么文件就自动关闭
                    isEnd.set(true);
                }
            } else {
                ...
                if (bufferEmptyCount >= (1000 / sleepMilis) * Indexer.closeDelay.get()) {
                    isEnd.set(true);
                ...
                if (future == null || (future != null && future.isDone())) {
                    ...
                    if (dataWriter != null) {
                        ...
                        while (true) {
                            //bufferSizeInfact = 0;
                            try {
                                tmpGenericRecord = (GenericRecord) this.inbuffer.poll(500, TimeUnit.MILLISECONDS);
                            } catch (Exception ex) {
                                log.error(ex, ex);
                            }
