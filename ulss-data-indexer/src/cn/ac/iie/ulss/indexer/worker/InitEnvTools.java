 ...
 * @author work
 */
public class InitEnvTools {

    public static Logger log = Logger.getLogger(InitEnvTools.class.getName());
        ...
        try {
            ...
            for (List<String> list : allSchema) {
                ...
                if (schemaName.equals("docs")) {
                    ...
                    Protocol protocol = Protocol.parse(Indexer.docsSchemaContent);
                    Indexer.docsSchema = protocol.getType(schemaName);
                    Indexer.docsReader = new GenericDatumReader<GenericRecord>(Indexer.docsSchema);
                    Indexer.schemaname2Schema.put("docs", Indexer.docsSchema);
    ...
    public static void initStatVolumeSchema() {
        Indexer.statVolumeSchemaContent = Indexer.imd.getStatVolumeSchema();
        Protocol protocol = Protocol.parse(Indexer.statVolumeSchemaContent);
        Indexer.statVolumeSchema = protocol.getType("data_volume_stat");
    }
    ...
    public static void initIndexinfo(ConcurrentHashMap<String, LinkedHashMap<String, List<FieldSchema>>> table2idx2colMap,
        ...
        try {
            ...
            FieldType fieldtype = InitEnvTools.getFieldType("true", "true", "long");
            ...
            for (Index idx : idxList) {
                if (idx.getIndexHandlerClass().equalsIgnoreCase("lucene")) {
                    ...
                    if (dataType.equalsIgnoreCase(Constants.DATA_TYPE_TIMESTAMP) || dataType.equalsIgnoreCase("array<timestamp>")) {
                        FieldType ft = InitEnvTools.getFieldType(lucIdx, lucStore, dataType);
                        ...
                        FieldType ft = InitEnvTools.getFieldType(lucIdx, lucStore, "long");
                        idx2LuceneFieldTypeMap.put(idx.getIndexName().toLowerCase(), ft);
                        field = new LongField(idx.getIndexName().toLowerCase(), -999, ft);
                        ...
                        FieldType ft = InitEnvTools.getBinaryFieldType(lucIdx, lucStore);
                        idx2LuceneFieldTypeMap.put(idx.getIndexName().toLowerCase(), ft);
                        byte[] val = new byte[1];
    ...
    public static void initIndexinfo() {
        for (int i = 0; i < Indexer.hlwrzthreadpoolSize * 2; i++) {
            ...
            for (String tableFullName : Indexer.tablename2Table.keySet()) {
                ...
                try {
                    ...
                    List<Index> idxList = Indexer.tablename2indexs.get(tableFullName);
                    for (Index idx : idxList) {
                        String idxName = idx.getIndexName().toLowerCase();
                        log.debug("index name is " + idxName);
                        List<FieldSchema> indexCols = idx.getSd().getCols();
            ...
            try {
                Indexer.table2idx2LuceneFieldMapPool.put(table2idx2LuceneFieldMap);
                Indexer.table2idx2LuceneFieldTypeMapPool.put(table2idx2LuceneFieldTypeMap);
                Indexer.table2idx2colMapPool.put(table2idx2colMap);
            } catch (Exception ex) {
                log.error(ex, ex);
            }
        }
