...
public class DataWriter implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            //try {
            this.table2idx2LuceneFieldMap = GlobalParas.table2idx2LuceneFieldMapPool.take();
            this.table2idx2colMap = GlobalParas.table2idx2colMapPool.take();
            this.table2idx2LuceneFieldTypeMap = GlobalParas.table2idx2LuceneFieldTypeMapPool.take();
            ...
            String schemaName = GlobalParas.tablename2Schemaname.get(this.tableName.toLowerCase());
            String schemaContent = GlobalParas.schemaname2schemaContent.get(schemaName);
            ...
            for (GenericRecord ms : innerDataBuf) {
                if (ms == null) {
                    ...
                    try {
                        docsRecord = ms;
                        GenericArray docSet = (GenericData.Array<GenericRecord>) docsRecord.get(GlobalParas.docs_set);
            ...
            log.info("write lucene file " + this.dbName + "." + this.tableName + " file_id:" + this.file_id + ": " + (System.currentTimeMillis() - bg) + " ms for " + writenum);
            writenum = 0;
            innerDataBuf.clear();
        } catch (Exception ex) {
            log.error(ex, ex);
            ...
            innerDataBuf.clear();
            try {
                GlobalParas.table2idx2LuceneFieldMapPool.put(this.table2idx2LuceneFieldMap);
                GlobalParas.table2idx2colMapPool.put(this.table2idx2colMap);
                GlobalParas.table2idx2LuceneFieldTypeMapPool.put(this.table2idx2LuceneFieldTypeMap);
