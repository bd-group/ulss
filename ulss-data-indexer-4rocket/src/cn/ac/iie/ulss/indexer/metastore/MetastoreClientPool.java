...
public class MetastoreClientPool {
    ...
    public MetastoreClientPool(int initialSize, String url, int port) {
        int i = 0;
        this.url = url;
        this.port = port;
        while (i < initialSize) {
            try {
                MetaStoreClient cli = new MetaStoreClient(this.url, this.port);
                if (cli != null) {
                    clientPool.add(cli);
                    i++;
                } else {
    ...
    public MetaStoreClient makesureGetOneNewClient() {
        MetaStoreClient tmp = MetastoreClientPool.makesureCreateMetaStoreClient(this.url, this.port);
        return tmp;
    }

    public MetaStoreClient getOneNewClient() {
        MetaStoreClient tmp = MetastoreClientPool.createMetaStoreClient(this.url, this.port);
        return tmp;
    }
    ...
    private static MetaStoreClient createMetaStoreClient(String url, int port) {
        try {
            return new MetaStoreClient(url, port);
        } catch (Exception e) {
    ...
    private static MetaStoreClient makesureCreateMetaStoreClient(String url, int port) {
